...
const createLogin = (db, online_users) => (metadata, meta_stream, cb) => {
  debug('::login::')
  ...
  // make sure metadata.user is the same as meta_stream.whoami
  // verify that metadata.password is the same as stored in db[user].password
  // where to persist logged in users
}

const createLogoff = (db, online_users) => (metadata, meta_stream, cb) => {
  // ...
}
