...
const HookedTtlMap = require('hooked-ttl-map') // TODO: publish
...
const createHandlePair = media_server => {
  ...
  const waiting = new HookedTtlMap()
  return (a, b) => {
    debug('::handlePair::')
    debug(`pair: ${a}, ${b}`)
    ...
    stream_preactor.onlyWithin(now, end).on('stream', (media_stream, req) => {
      media_stream.once('data', chunk => {
        ...
        parseJSON(chunk, (err, info) => {
          ...
          const pair = pairs.find(s => s.has(info.user) && s.has(info.peer))
          ...
          if (!pair) {
            ...
            // FIXME: x-pump not yet working...
            debug(`destroyed states before cross-pumping; ` +
                  `media_stream.destroyed: ${media_stream.destroyed}; ` +
                  `peer_stream.destroyed: ${peer_stream.destroyed}`)
            // pump(peer_stream, media_stream, handleError) // data !flows thru
            // pump(media_stream, peer_stream, handleError) // data flows thru
            peer_stream.resume().on('data', chunk => {
              debug(`peer_stream -> media_stream chunk: ${String(chunk)}`)
              media_stream.write(chunk, handleError)
            })
            media_stream.on('data', chunk => {
              debug(`media_stream -> peer_stream chunk: ${String(chunk)}`)
              peer_stream.write(chunk, handleError)
            })
          } else {
            debug(`${info.user} about to await ${info.peer} for 10s`)
            media_stream.pause()
            waiting.set(10000, info.user, media_stream, (k, v, doDelete) => {
              debug(':willDelete:')
              debug(`stream.paired: ${v.paired}`)
              if (!v.paired) v.destroy()
              debug(`stream.destroyed after "paired" check: ${v.destroyed}`)
              doDelete()
            })
          }
