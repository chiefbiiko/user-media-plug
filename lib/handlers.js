...
const HookedTimeToLiveMap = require('hooked-ttl-map')
...
const { levelHasAllKeys } = require('./utils.js')
...
const ERR = {
  ...
  notFoundInDB (keys) {
    return Error(`some db keys could not be found: ${JSON.stringify(keys)}`)
  }
}
...
const createRegisterUser = db => (metastream, metadata, cb) => {
  ...
  db.get(metadata.user, err => {
    ...
    const new_user = {
      ...
      peers: [],
      status: 'noop',
      avatar: null // TODO: default avatar!!!
    }
...
const createAvatar =
  (db, active_metastreams, forward) => (metastream, metadata, cb) => {
  debug('::avatar::')
...
const createAddPeers = db => (metastream, metadata, cb) => {
  ...
  levelHasAllKeys(db, metadata.peers, (err, truth) => {
    if (err) {
      metastream.write(o(outbound.res(metadata.type, metadata.tx, false)))
      return cb(err)
    } else if (!truth) {
      metastream.write(o(outbound.res(metadata.type, metadata.tx, false)))
      return cb(ERR.notFoundInDB(metadata.peers))
    }
...
const createStopRinging = forward => (metastream, metadata, cb) => {
  debug('::stopRinging::')
...
const createGetPeers = (db, logged_in_users) => (metastream, metadata, cb) => {
  ...
  if (!valid.schema_GET_PEERS_USER(metadata)) {
    metastream.write(o(outbound.res(metadata.type, metadata.tx, false)))
    return cb(ERR.invalidSchema('GET_PEERS_USER', metadata))
  }
...
const createGetUser = db => (metastream, metadata, cb) => {
  debug('::getUser::')
  ...
  db.get(metadata.user, (err, user) => {
    ...
    const res_user = outbound.res(metadata.type, metadata.tx, true, {
      user: {
        ...user,
        password: undefined
      }
    ...
    metastream.write(o(res_user), cb)
  })
}
...
const createHandleMetadata = (msg_handlers, logged_in_users) => {
  ...
  return (metastream, metadata) => {
    ...
    switch (metadata.type) {
      ...
      case 'GET_USER': getUser(metastream, metadata, handleError); break
      case 'STATUS': status(metastream, metadata, handleError); break
      case 'CALL': call(metastream, metadata, handleError); break
      case 'STOP_RINGING': stopRinging(metastream, metadata, handleError); break
...
const createHandleMetastream = injects => {
  ...
  return (metastream, req) => {
    ...
    metastream.on('data', createHandleMetadata({
      ...
      avatar: createAvatar(db, active_metastreams, forward),
...
const createUnpair = (metaserver, forward) => (metastream, metadata, cb) => {
  debug('::unpair::')
  ...
  forward(metadata, [ metadata.peer ], error => {
    metastream.write(o(outbound.res(metadata.type, metadata.tx, true)), err => {
...
const createHandleRequest = () => async (req, res) => {
  debug('::handleRequest::')

  // if ([ '/meta', '/media' ].includes(parse(req.url).pathname)) return

  const serve_conf = {
    public: join(module.parent.__dirname, 'view', 'build')
  }

  await serveHandler(req, res, serve_conf)
}
