...
const createWhoami = active_meta_streams => (meta_stream, metadata, cb) => {
  debug('::whoami::')
...
const createHandleMetadata = (handlerMap, logged_in_users) => {
  ...
  return (meta_stream, metadata) => {
    ...
    switch (metadata.type) {
      case 'WHOAMI': whoami(meta_stream, metadata, handleError); break
...
const createHandleMetastream = injects => {
  ...
  return (meta_stream, req) => {
    ...
    }, logged_in_users).bind(null, meta_stream))
  }
}
