        ...
        media_stream.once('data', chunk => {
          ...
          parseJSON(chunk, (err, info) => {
            ...
            if (!paired) {
              ...
              // hashtag-stream-set (inherits from stream-set)
              // hashtagAdd(tag, ...streams), hashtagRemove(tag[, willDelete(stream, doDelete)])
              // hashtags streams on add: const hashtag = (...words) => `#${words.sort().join('-')}`
              // iterate set and remove streams with their hashtag equal to `tag` on remove
              // listen to stream_set's 'remove' event to iterate it and remove all streams with the same hashtag
...
const createUnpair = active_media_streams => (meta_stream, metadata, cb) => {
  ...
  debug(`hashtag: ${tag}, deleted: ${deleted}`)
  meta_stream.write(o(outbound.res(metadata.type, metadata.tx, deleted)), cb)
}

// const createHandleUnpair = active_media_streams => (a, b) => {
//   debug('::handleUnpair::')
//
//
// }
...
const willDeleteMediaStreams = (tag, fading_streams, doDelete) => {
  ...
  fading_streams.forEach(stream => stream.destroy())
  doDelete()
}
